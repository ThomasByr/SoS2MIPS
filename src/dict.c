...
static const unsigned prime_1 = 3079;
static const unsigned prime_2 = 1572869;
  ...
  if (dict->nitems + dict->n_deleted_items >= (double)dict->capacity * 0.85) {
    ...
    for (ii = 0; ii < old_capacity; ii++) {
      if (old_keys[ii] != 0 && old_keys[ii] != 1) {
        _dict_push_item(dict, (void *)old_keys[ii], (void *)old_values[ii]);
      }
    }
    free(old_keys);
    free(old_values);
...
size_t dict_get(dict_t dict, void *key) {
  if (dict == NULL || key == NULL) return 0; // return error
  ...
  if (dict == NULL || k == 0 || k == 1) return 0; // return error
...
int dict_discard(dict_t dict, void *item) {
  if (dict == NULL || item == NULL) return 0; // return error
  ...
  // if dict is NULL or key casts to 0 or 1
  if (dict == NULL || key == 0 || key == 1) return -1; // return error
...
dict_t dict_copy(dict_t dict) {
  ...
  if (new_dict->keys == NULL || new_dict->values == NULL) {
    if (new_dict->keys != NULL) free(new_dict->keys);
    if (new_dict->values != NULL) free(new_dict->values);
...
size_t dict_merge(dict_t dict1, dict_t dict2) {
  ...
  if (dict1 == NULL || dict2 == NULL || dict1 == dict2) return 0;

  size_t ii; // index of the bucket
  for (ii = 0; ii < dict2->capacity; ii++) {
    if (dict2->keys[ii] != 0 && dict2->keys[ii] != 1) {
      int rv =
          dict_push(dict1, (void *)dict2->keys[ii], (void *)dict2->values[ii]);
      ASSERT(rv == 0 || rv == 1);
      if (rv == 1) count++;
    }
  }
...
dict_itr_t dict_itr_new(dict_t dict) {
  dict_itr_t itr = (struct dict_itr_s *)calloc(1, sizeof(struct dict_itr_s));
...
int dict_itr_has_next(dict_itr_t itr) {
  ...
  if (itr->dict->nitems == 0 || itr->index >= itr->dict->capacity) return 0;
...
size_t dict_itr_next(dict_itr_t itr) {
  if (dict_itr_has_next(itr) == 0) return -1;
...
void *dict_itr_for_each(dict_itr_t itr, for_each_callback_t *fe1, void *data1,
  ...
  if (fe1 == NULL && fe2 == NULL) return NULL;
...
void dict_itr_discard_all(dict_itr_t itr, delete_callback_t *dc1,
  ...
  while (dict_itr_has_next(itr)) {
    ...
    if (key != NULL) {
      ...
      if (dc1 != NULL) (dc1)(key);
      if (value != NULL && dc2 != NULL) (dc2)(value);
    }
    dict_itr_next(itr);
  }
}
